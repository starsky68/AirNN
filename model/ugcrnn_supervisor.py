...
class UGCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        ugcrnn_model = UGCRNNModel(adj_mx, self._logger, **self._model_kwargs) # 实例化一个GCRNN模型对象

        self.ugcrnn_model = ugcrnn_model.cuda() if torch.cuda.is_available() else ugcrnn_model # 将模型放到GPU上（如果可用的话）
    ...
    def save_model(self, epoch):
        ...
        config['model_state_dict'] = self.ugcrnn_model.state_dict() # 存储网络结构的名字和对应的参数
    ...
    def load_model(self):
        ...
        self.ugcrnn_model.load_state_dict(checkpoint['model_state_dict']) # 将模型参数 加载到模型中
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            # 作用：tensorboard可视化使用
            self._writer.add_scalar('{} loss'.format(dataset), min_loss, batches_seen) # 变量名、存放的值、x坐标
    ...
    def _train(self, base_lr,
        ...
        wait = 0
        optimizer = torch.optim.Adam(self.ugcrnn_model.parameters(), lr=base_lr, eps=epsilon) # Adam优化器
        ...
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=steps,   # 动态调整学习率的函数
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            self.ugcrnn_model = self.ugcrnn_model.train() # 打开训练模式
            ...
            for _, (x, y) in enumerate(train_iterator): # train_iterator是一个生成器，可以迭代，每次迭代则切片 取一次数据，好处是节约内存啊
                ...
                #print(x.size(),'xxxxx')
                #print(y.size(),'yyyyy')
                output = self.ugcrnn_model(x, y, batches_seen) # 调用模型，得出预测结果，其中batches_seen为：迄今为止 经过的 批次
                #print(output.size(), 'oooo')
                self._plot_regres(x,y,output)
                if batches_seen == 0:
                ...
                if is_bnn:
                    kl_loss = self._compute_klloss(self.ugcrnn_model)  # bnn的话使用klloss
                    loss+=kl_loss.item()
            ...
            if val_loss < min_val_loss: # 若当前的验证损失 < 最小验证损失，则保存当前的模型参数
                ...
                if wait == patience: # 这个就是说，当训练了patience次，每次的验证训练损失都比前一次大，那就终止训练
                    ...
                    break
    def _plot_regres(self,x,y,pre):
        from matplotlib import pyplot as plt
        from scipy.spatial import ConvexHull
        #fig = plt.figure()
        #ax = fig.add_subplot(111,projection='3d')
        x,y,pre = x.reshape(-1),y.reshape(-1),pre.reshape(-1)

        x, y, pre = x.cpu().detach().numpy(),y.cpu().detach().numpy(),pre.cpu().detach().numpy()

        #x,y,pre = np.meshgrid(x,y)
        #ax.plot_trisurf(x,y,pre,cmap='rainbow')
        plt.scatter(x, y,c='b')
        plt.scatter(x, pre,c='g')
    ...
    def _compute_klloss(self, model, kl_weight=0.0001):
        ...
        kl = kl_loss(model)
        kl = 1/kl
        return kl_weight * kl # 调用了计算带mask的mae损失的函数
